...
unet.train().to(accelerator.device)
...
unet, generator_optimizer, train_loader = accelerator.prepare(unet, generator_optimizer, train_loader)
    ...
    for i, pack in enumerate(train_loader, start=1):
        with accelerator.accumulate(unet):
            for rate in size_rates:
                ...
                loss1 = F.mse_loss(mask_latent.to(accelerator.device).to(weight_dtype),mask_latents.to(accelerator.device).to(weight_dtype), reduction="mean")
                loss2 = F.mse_loss(edge_latent.to(accelerator.device).to(weight_dtype),edge_latents.to(accelerator.device).to(weight_dtype), reduction="mean")
                loss = loss1 + loss2
