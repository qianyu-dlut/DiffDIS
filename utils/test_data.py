...
class test_dataset:
    def __init__(self, image_root, gt_root):
        ...
        # self.img_list_1 = [os.path.splitext(f)[0] for f in os.listdir(image_root) if '18#Plant#2#Tree#37197398761_9f351fcb47_o'  in f]
        # self.img_list_1 = [os.path.splitext(f)[0].replace('_pred', '') for f in os.listdir(image_root) if f.endswith('.png') or f.endswith('.jpg') or f.endswith('.bmp')]
    ...
    def load_data(self):
        ...
        # rgb_bmp_path = 
        if os.path.exists(os.path.join(self.image_root,self.img_list[self.index]+ '.png')):
            image = self.binary_loader(os.path.join(self.image_root,self.img_list[self.index]+ '.png'))
        elif os.path.exists(os.path.join(self.image_root,self.img_list[self.index]+ '.jpg')):
            image = self.binary_loader(os.path.join(self.image_root,self.img_list[self.index]+ '.jpg'))
        else:
            image = self.binary_loader(os.path.join(self.image_root,self.img_list[self.index]+ '_pred.bmp'))
        ...
        self.index += 1
        return image, gt, os.path.join(self.gt_root, self.img_list[self.index - 1] + ".jpg")
